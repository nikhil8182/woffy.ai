...
const ChatWithWoffy = () => {
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      text: "Demo initialized. Hello! I am Woffy, your project guide. Ready to assist.",
      sender: 'woffy',
      mode: 'intro',
  ...
  const sendRequest = async (request) => {
    ...
    try {
      ...
      const responseId = ++messageId.current;
      ...
      setRequestError(error instanceof Error ? error.message : 'Unable to reach Woffy. Please try again.');
    } finally {
  ...
  const handleSendMessage = (e, text = inputText) => {
    if (e) e.preventDefault();
    if (requestInFlight.current || !text.trim()) return;
    const question = text.trim();
    const history = messages.filter(message => message.mode !== 'intro').slice(-12).map(message => ({
  ...
  const handleRetry = () => {
    if (failedRequest.current && !requestInFlight.current) sendRequest(failedRequest.current);
  };
    ...
    <>
      ...
      <div className={`h-screen bg-slate-950 text-slate-200 font-sans selection:bg-indigo-500/30 overflow-hidden flex flex-col fixed inset-0 z-50 transition-opacity duration-1000 ${bootComplete ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="relative z-10 flex-1 flex overflow-hidden">
                ...
                <div className="px-6 py-4 space-y-6 flex-1 overflow-y-auto custom-scrollbar-dark">
                  ...
                  <div className="space-y-3">
                    <h3 className="text-[9px] font-bold text-slate-600 uppercase tracking-[0.2em] flex items-center gap-2">
                      <Aperture size={10} /> Concept subsystems
                    </h3>
                    ...
                    <p className="text-[10px] leading-relaxed text-slate-500">Illustrated indicators. No physical robot is connected.</p>
                  </div>
                  ...
                  <div className="bg-black/30 rounded-xl p-3.5 border border-white/[0.04] font-mono text-[9px] text-slate-500 space-y-2 shadow-inner">
                    <div className="text-slate-400 border-b border-white/[0.05] pb-1.5 mb-1.5 tracking-[0.15em] flex items-center gap-1.5">
                      <Activity size={8} className="text-indigo-400" /> DEMO EVENT LOG
                    </div>
                    {[
          ...
          <motion.button
            type="button"
            aria-label={sidebarOpen ? 'Hide demo sidebar' : 'Show demo sidebar'}
          ...
          <div className="flex-1 flex flex-col relative bg-slate-950/30 h-full">
            ...
            <div className="h-14 border-b border-white/[0.05] flex items-center justify-between px-6 bg-slate-900/40 backdrop-blur-xl flex-shrink-0 relative">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <a href="/" aria-label="Return to Woffy home" className="font-semibold text-white tracking-wide text-sm hover:text-indigo-300">Woffy Demo ↗</a>
                </div>
              </div>
            ...
            {/* Messages Area */}
            <div role="log" aria-label="Conversation with Woffy" aria-live="polite" aria-relevant="additions" className="flex-1 overflow-y-auto p-4 lg:p-8 space-y-5 scroll-smooth custom-scrollbar-dark">
                  ...
                  >
                    <div className={`flex gap-3 max-w-[85%] lg:max-w-[70%] ${msg.sender === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                      ...
                      <div className={`flex flex-col gap-1.5 ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                        {msg.sender === 'woffy' && msg.mode !== 'intro' && (
                          <span className="text-[10px] text-indigo-300/80 font-mono px-1">
                            {msg.mode === 'project-faq' ? 'PROJECT FAQ · AI temporarily unavailable' : 'AI REPLY'}
                          </span>
                        )}
                        <motion.div
                          whileHover={{ y: -1 }}
            ...
            <div className="p-4 lg:p-6 flex-shrink-0 z-30 relative">
              ...
              <div className="max-w-4xl mx-auto relative">
                ...
                {requestError && (
                  <div role="alert" className="mb-3 flex items-center justify-between gap-3 rounded-xl border border-rose-400/20 bg-rose-500/5 px-4 py-3 text-xs text-rose-200">
                    <span>{requestError}</span>
                    <button type="button" onClick={handleRetry} disabled={woffyState === 'thinking'} aria-label="Retry last message" className="shrink-0 rounded-lg border border-rose-300/25 px-3 py-1.5 font-medium hover:bg-rose-400/10 disabled:opacity-40">Retry</button>
                  </div>
                )}
                ...
                <div className="text-center mt-3 text-[10px] text-slate-500 font-mono tracking-wide flex flex-wrap items-center justify-center gap-2">
                  <span>Cloud AI demo</span>
                  <span className="w-1 h-1 rounded-full bg-slate-700" />
                  <span>Avatar and indicators are illustrative</span>
                </div>
              </div>
            </div>
